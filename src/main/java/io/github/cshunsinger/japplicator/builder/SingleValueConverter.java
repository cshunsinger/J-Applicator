...
public class SingleValueConverter {
    public static CodeInsnBuilderLike createValueConverter(CodeInsnBuilderLike valueBuilder, Class<?> sourceType, Class<?> destType) {
        ...
        if(sourceType.isPrimitive()) {
            Class<?> sourceWrapper = primitiveToWrapper(sourceType);
            if(destType.isAssignableFrom(sourceWrapper))
                return cast(destType, numericToNumericConversionStep(valueBuilder, sourceType, sourceWrapper));
        }
        ...
        if(destType.isAssignableFrom(String.class))
            return toStringConversionStep(valueBuilder, sourceType, destType);

        //If this line is reached, type conversion was not possible
        throw new IllegalArgumentException("No conversion exists from type %s to type %s.".formatted(
